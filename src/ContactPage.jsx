...
const INITIAL_FORM = { type: 'General question', email: '', message: '' }
  ...
  return (
    <div className="min-h-screen bg-[#f0fafa]">
      ...
      <div className="max-w-2xl mx-auto px-4 md:px-6 py-8 md:py-16">
        <h1 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">Contact & Support</h1>
        ...
        {/* Quick-select type cards */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4 mb-8 md:mb-10"
          role="group" aria-label="Select message type">
          {CONTACT_TYPES.map((item) => (
            <button key={item.id} type="button"
              onClick={() => { setForm((prev) => ({ ...prev, type: item.title })); if (formError) setFormError('') }}
              aria-pressed={form.type === item.title}
              className={`text-left bg-white rounded-2xl border p-4 transition-all ${
                form.type === item.title ? 'border-[#0a9396] shadow-sm' : 'border-gray-100 hover:border-gray-200'
              }`}>
        ...
        {status === 'success' ? (
          <div className="bg-white rounded-2xl p-8 md:p-10 text-center border border-gray-100 shadow-sm">
            ...
            <h2 className="text-lg md:text-xl font-bold text-gray-900">Message sent!</h2>
            <p className="text-gray-500 mt-2 text-sm md:text-base">We'll get back to you within 48 hours.</p>
          ...
          <form onSubmit={handleSubmit} noValidate
            ...
            <div>
              <label htmlFor="contact-type" className="text-sm font-semibold text-gray-700 mb-1 block">
                What's this about?
              </label>
            ...
            <div>
              <label htmlFor="contact-email" className="text-sm font-semibold text-gray-700 mb-1 block">Your email *</label>
              <input id="contact-email" required type="email" name="email" value={form.email} onChange={handleChange}
            ...
            <div>
              <div className="flex justify-between items-baseline mb-1">
                ...
                <span className={`text-xs ${charsLeft < 100 ? 'text-orange-500' : 'text-gray-400'}`}
                  aria-live="polite" aria-label={`${charsLeft} characters remaining`}>
                  {charsLeft} / {MAX_MESSAGE_LENGTH}
                </span>
              </div>
              <textarea id="contact-message" required name="message" value={form.message} onChange={handleChange}
                rows={5} maxLength={MAX_MESSAGE_LENGTH} placeholder="Tell us anything..."
