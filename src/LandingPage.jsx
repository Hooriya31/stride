...
// ─── Count-up hook — animates a number from 0 to target when in view ─────────
...
function StatCard({ stat, label, desc }) {
  const isNumeric = /^\d+$/.test(stat)
  const { value, ref } = useCountUp(isNumeric ? stat : 0)

  return (
    <div ref={ref} className="bg-white/10 rounded-2xl p-6 text-white">
      <h3 className="text-3xl font-bold">
        {isNumeric ? `${value}+` : stat}
      </h3>
      <p className="font-semibold mt-1">{label}</p>
      <p className="text-sm text-[#d0f0f0] mt-2">{desc}</p>
    </div>
  )
}
...
function LandingPage() {
  ...
  useEffect(() => {
    async function fetchOpportunities() {
      ...
      try {
        ...
        const merged   = [...featured, ...latest.filter((o) => !seen.has(o.id))]
  ...
  // Hero preview: featured-first, then soonest deadline
  ...
  // Featured section: same priority
  ...
  const ABOUT_STATS = [
    { id: 'opps',    stat: '500', suffix: '+', label: 'Opportunities', desc: 'Scholarships, internships, fellowships, competitions and more' },
    { id: 'cats',    stat: '7',   suffix: '',  label: 'Categories',    desc: "Organized so you find exactly what you're looking for"      },
    { id: 'updates', stat: null,  suffix: '',  label: 'Weekly Updates',desc: 'New opportunities added regularly so you never miss out'    },
    { id: 'cost',    stat: null,  suffix: '',  label: '0 PKR',         desc: 'Completely free for Pakistani students'                      },
  ]
  ...
  return (
    <div className="min-h-screen bg-[#f0fafa] text-gray-900">
      ...
      {/* Navbar */}
      <nav aria-label="Main navigation" className="sticky top-0 z-30 bg-white/90 backdrop-blur shadow-sm">
        <div className="max-w-6xl mx-auto px-6 md:px-10 py-4 flex items-center justify-between">
          <Logo />
          <div className="flex items-center gap-3">
            <Link to="/auth" className="text-sm font-medium text-gray-600 border border-gray-200 px-4 py-2 rounded-full hover:border-[#0a9396] hover:text-[#0a9396] transition-all">
              Sign In
            </Link>
            <Link to="/auth" className="text-sm font-semibold bg-[#0a9396] text-white px-5 py-2 rounded-full hover:bg-[#007f82] transition-all">
              Get Started
            </Link>
          </div>
        </div>
      ...
      {/* ── Hero ─────────────────────────────────────────────────────────── */}
      <section className="px-6 md:px-10 pt-16 md:pt-20 pb-16"
        ...
        <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-4 mt-8">
              <Link to="/auth" className="bg-[#0a9396] hover:bg-[#007f82] text-white px-8 py-3 rounded-full font-semibold transition-all shadow-lg">
                Explore Stride
              </Link>
          ...
          {/* Right: real dynamic preview cards */}
          <div className="relative">
            ...
            <div className="relative bg-[#f8ffff] rounded-3xl shadow-xl border border-gray-100 p-5">
              {loading ? (
      ...
      <section id="featured" className="max-w-6xl mx-auto px-6 md:px-10 py-14">
        ...
        {fetchError && (
          <div role="alert" className="text-center py-6 text-red-500 text-sm">{fetchError}</div>
        )}
        ...
        <div className="text-center mt-10">
          <Link to="/auth" className="inline-block bg-[#0a9396] hover:bg-[#007f82] text-white px-8 py-3 rounded-full font-semibold transition-all">
      ...
      <section className="bg-[#0a9396] py-20 px-6 md:px-10">
        <div className="max-w-6xl mx-auto text-center">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-12 mb-12">
            {[
              { id: 's1', stat: '500', label: 'Opportunities', isNumeric: true  },
              { id: 's2', stat: '7',   label: 'Categories',    isNumeric: true  },
              { id: 's3', stat: null,  label: 'Free Forever',  isNumeric: false },
              { id: 's4', stat: null,  label: 'Weekly Updates',isNumeric: false },
            ].map((item) => {
              if (!item.isNumeric) {
                return (
                  <div key={item.id} className="bg-white/10 rounded-2xl p-5 text-white">
                    <p className="text-3xl font-bold">✓</p>
                    <p className="text-sm font-semibold mt-1">{item.label}</p>
                  </div>
                )
              }
              return <AnimatedStat key={item.id} stat={item.stat} label={item.label} />
            })}
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-6 text-left">
            {[
              ...
              { id: 'grows',   title: 'Built to grow with you',   desc: 'Start with opportunities. Track deadlines, save favourites, submit new ones for the community.' },
            ].map((item) => (
      ...
      <section className="max-w-6xl mx-auto px-6 md:px-10 py-20">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {[
            ...
            <div key={step.id} className="bg-white rounded-2xl border border-gray-100 p-8 hover:shadow-md transition-all">
      ...
      <section className="px-6 md:px-10 pb-20">
        <div className="max-w-6xl mx-auto relative overflow-hidden bg-[#0a9396] rounded-3xl p-10 md:p-14 text-center shadow-xl">
          {/* Decorative blobs */}
          ...
          <div className="relative">
            <span className="inline-block text-sm font-semibold text-white/80 bg-white/10 px-4 py-1 rounded-full mb-6">
              Free for every Pakistani student
            </span>
            <h2 className="text-3xl md:text-4xl font-bold text-white">
              Your next opportunity is probably already on Stride.
            </h2>
            <p className="text-white/80 mt-4 max-w-2xl mx-auto text-base md:text-lg">
              Join Stride to browse opportunities, track deadlines, and never miss the ones that actually matter.
            </p>
            <Link to="/auth"
              className="inline-block mt-8 bg-white text-[#0a9396] px-8 py-3 rounded-full font-bold hover:bg-gray-50 transition-all shadow-lg text-base">
              Get started free →
            </Link>
          </div>
        </div>
...
// Separated so useCountUp hook can work per-card
function AnimatedStat({ stat, label }) {
  const { value, ref } = useCountUp(stat)
  return (
    <div ref={ref} className="bg-white/10 rounded-2xl p-5 text-white">
